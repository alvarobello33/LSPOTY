...
L'usuari ha de poder eliminar el seu perfil. Ha de saltar un avis avisant que l'eliminacio es permanent. Si es borra el perfil, també es borraran les playlists i tota la info i tal..
...
## IMPORTANT 
S'ha de crear la nostra propia API i fer amb AJAX (aixo no es pot fer amb Forms ja que no accepten PUT ni DELETE.)
...
S'ha de fer servir el nou environment (ens el donen a la carpeta.)

S'ha de fer servir GIT -> Cada membre de grup ha de fer com a mínim 10 commits representatius.

El tema de gestió de la base de dades -> S'ha de fer amb migracions, haurem d'entregar la carpeta amb totes les migracions. (No ens donen SQL).

La web ha d'estar traduida a dos idiomes: Angles i el que vulgueum (catala o castella). S'ha de traduir automaticament. No sha de fer selector d'idioma, el sistema ha de traduir la web automaticament en funcio del navegador d l'usuari.

Recordar ficar el fitxer .env com a "Production" no com a "Development".

Recordar fer el TAG (que diu l'enunciat) quan pengem el projecte final al git.

El .env de www penjar-lo a gitlab, pero el .env del docker no s'ha de penjar al git. (Posarho al gitignore).

gitignore.io -> Et genera el gitignore li poses: codeigniter, visual studio, phpstorm, composer, windows, mac, etc.
El fitxer s'ha d posar al root.